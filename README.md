...
| Paper item | Command |
|---|---|
| Fig. 4, Tab. 6, App. E.6–E.8 (operating-point sweeps) | `bash reproduce.sh ablation_<cifar\|vit\|nlp\|lora>` (`ablation_nlp` includes the E.6 X=0.5 matched-SE cell, `ANCHOR_SE=0.5 PA_VALUES=0.5 bash scripts/experiments/nlp/ablation_pa.sh`); plot: `python scripts/plot_ablation_curves.py` |
...
| App. E.12–E.14 (LoRA diagnostics) | `python scripts/diagnose_lora_specialization.py --run_dir outputs/rtx5090_lora_faithful/ours_s<seed>`, `python scripts/eval_lora_per_task.py --run_dir … --method ours`, `python scripts/lora_per_task_fisher.py`. Soft SpecDrop is evaluated at the routing state with which the run's reported checkpoint was scored (per-step $p_a$ warmup at the end of `results.json`'s `best_epoch`); the un-ablated baseline must equal `results.json` `eval_rouge_l` exactly on the same GPU type. |
| App. E.15 (embedding diagnostics) | BGE embeddings and k=7 clusters of the 100M-token cache: `python -m data.cluster_chunks --train-cache <tokenized_train_seq512_tok100000000_*.pt> --train-output data_cache/slimpajama/clusters_train_seq512_tok100000000_bge-large_k7.pt --val-cache <tokenized_val_seq512_tok5000000_*.pt> --val-output data_cache/slimpajama/clusters_val_seq512_tok5000000_bge-large_k7.pt --embedder BAAI/bge-large-en-v1.5 --n_clusters 7`; then `python scripts/select_optimal_k.py`, `python scripts/embed_cifar100.py`, `python scripts/intra_chunk_heterogeneity.py`, `python scripts/analyze_purity_ppl_correlation.py` |
